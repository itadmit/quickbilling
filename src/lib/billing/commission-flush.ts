...
export async function flushCommissions(): Promise<{
  ...
  for (const g of groups) {
    ...
    // Fetch the PDF generated by PayPlus's Invoice+ for this charge.
    const docs = charge.transactionUid
      ? await getInvoiceDocuments(charge.transactionUid)
      : { primaryUrl: undefined, primaryUuid: undefined };
    const docUuid = docs.primaryUuid ?? charge.invoiceUuid;
    const docUrl = docs.primaryUrl ?? charge.invoiceUrl;
